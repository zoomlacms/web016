<%@ page language="C#" autoeventwireup="true" inherits="ZoomLa.WebSite.User.Content.MyContentpage, App_Web_mvwzqnrx" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head runat="server">
<title>我的内容</title>
...
<asp:Label ID="lblAddContent" runat="server" Text=""></asp:Label><a href="/User/Content/MyContent.aspx?NodeID=<%=Request["NodeID"]%>&status=-2"> 回收站</a>
</div>
<div class="us_pynews">
		...
		<Columns>
			...
			<asp:TemplateField HeaderText="操作">
				<ItemTemplate>
                    ...
                    <%--<asp:LinkButton ID="DeleteBtn" runat="server" Visible='<%#GetIsRe(Eval("Status", "{0}")) %>' CommandName="Delete" CommandArgument='<%# Eval("GeneralID") %>'
						OnClientClick="return confirm('你确定将该数据彻底删除？')">彻底删除</asp:LinkButton>         <a href="/User/Content/MyContent.aspx?NodeID=1&method=Del&ID=<%# Eval("GeneralID") %>">删除</a>
                        <a href="/User/Content/MyContent.aspx?NodeID=1&method=Reset&ID=<%# Eval("GeneralID") %>">还原</a>
                        <a href="/User/Content/MyContent.aspx?NodeID=1&method=Delete&ID=<%# Eval("GeneralID") %>">彻底删除</a>
                        --%>                   
				</ItemTemplate>
				<ItemStyle HorizontalAlign="Center" />
			</asp:TemplateField>
