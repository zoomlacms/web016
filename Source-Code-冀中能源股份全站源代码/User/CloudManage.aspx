<%@ page language="C#" autoeventwireup="true" inherits="User_Cloud_CloudManage, App_Web_sw1ttb1p" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html>
<html>
...
<div class="us_seta" id="us_seta" style="text-align: left;" runat="server">
	<input id="back" type="button" runat="server" class="C_input" style="margin-top: 5px;	margin-left: 5px;" value="返 回" onclick="javascript:history.back();" />
	<input type="button" class="C_input" name="demoCode04-3" value="上传" onclick="NewFormd('文件上传','Uploadify.aspx?content=MaxFileSize=1024000,Dir=<%=Server.UrlEncode(CurrentDir)%>,UploadFileExt=<%=UploadType %>')" />
	<input id="newFile" type="button" class="C_input" value="新建文件夹" onclick="NewFormd('新建文件夹','NewFolder.aspx?Dir=<%=Server.UrlEncode(CurrentDir)%>')" />
</div>
	...
	<ItemTemplate>
		<tr style="height: 24px" class="tdbg" onmouseover="this.className='tdbgmouseover';"
			...
			<%# System.Convert.ToInt32(Eval("type")) == 1 ? "" : "[<a  href='" + DownUrl + GetPath() + "'>下载</a>] [<a href='javascript:' onclick='javascript:parent.NewFormd(\"文件分享\" ,\"/Common/File.aspx?T=f&FD=" + FileJiaM(CurrentDir+"/"+Eval("Name").ToString()) + "\")'>分享</a>]"%>
			 [<asp:LinkButton ID="LbtnDelList" CommandName='<%# System.Convert.ToInt32(Eval("type")) == 1 ? "DelDir":"DelFiles" %>' CommandArgument='<%# Eval("Name")%>' OnClientClick="if(!this.disabled) return confirm('确定要删除吗？');" runat="server">删除</asp:LinkButton>]</td>
		</tr>
	</ItemTemplate>
</asp:Repeater>
<tr class="tdbg">
