<%@ page language="C#" autoeventwireup="true" inherits="manage_Plus_SurveyViewer, App_Web_o53mvtm5" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html>
<html>
<head runat="server">
...
<div class="r_navigation">
    后台管理&gt;&gt;<a href="SurveyManage.aspx">问卷投票管理</a>&gt;&gt;问卷来源分析
</div>
<asp:GridView ID="gviewSurSorcer" DataKeyNames="Rid" CellPadding="2" CellSpacing="1" BackColor="White" CssClass="border" GridLines="None" runat="server" AutoGenerateColumns="False" AllowPaging="True" PageSize="10" Width="100%" OnRowDataBound="gviewSurSorcer_RowDataBound" OnPageIndexChanging="gviewSurSorcer_PageIndexChanging" OnRowCommand="gviewSurSorcer_RowCommand">
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="提交者">
            <ItemTemplate>
                <asp:HyperLink ID="hlnkUser" Target="_self" NavigateUrl='<%# "UserVote.aspx?SID="+Eval("Sid")+"&UID="+Eval("Userid")+"&PTime="+Server.UrlEncode(Eval("Submitdate","{0}")) %>' ToolTip="查看我的答卷" runat="server"><%# GetUserName(Eval("Userid", "{0}"))%></asp:HyperLink>
                <a href='<%# "UserVote.aspx?SID="+Eval("Sid")+"&UID="+Eval("Userid")+"&PTime="+Server.UrlEncode(Eval("Submitdate","{0}")) %>' ></a> 
        ...
        <asp:TemplateField HeaderText="操作">
            <ItemTemplate>
                <asp:LinkButton ID="lbtnExport" runat="server" ToolTip="导出为Word文档" CommandName="Export" CommandArgument='<%# Eval("Rid") %>'>存入本地</asp:LinkButton> |
                <asp:LinkButton ID="lbtStatus" runat="server" ToolTip="审核" CommandName="Status" CommandArgument='<%# Eval("Rid") %>'>审核</asp:LinkButton> |
