<%@ page language="C#" autoeventwireup="true" inherits="MIS_AddDaily, App_Web_lrsx2ppt" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html> 
<html>
<head runat="server">
...
<th width="100"></th> <th>详细内容</th> <th width="180">编辑时间</th> </tr>
<tr><td width="100" align="center" valign="top">今日计划 </td><td id="log_box" colspan="2">
<div id="mylog_title_div" ><a href="javascript:void(0);"  onclick="ShowHidDiv('mylog_title_div','edit_middel_div')"> >>工作记录</a></div>
<div id="edit_middel_div" runat="server" style="display:none;" class="bgyellow"><textarea id="log_1" name="log_1" cols="60" rows="6" runat="server"></textarea><br /> 
...
<asp:Button ID="btnEdDay" CssClass="C_inputs" CommandArgument='<%#Eval("ID") %>' CommandName="Edit"   runat="server" Text="确定" />
<input id="Button5" type="button" class="C_inputs" onclick="ShowDiv('Dayshow_<%#Eval("ID") %>')"  value="取消" />
</li>
<li id='DayChang_<%#Eval("ID") %>' style="display:none;">
<input id='edtDate_<%#Eval("ID") %>'  onclick="WdatePicker({ dateFmt: 'yyyy/MM/dd HH:mm:ss' });" class="M_input"  />
<asp:Button ID="Button4" CssClass="C_inputs" CommandArgument='<%#Eval("ID") %>' CommandName="EditDate"   runat="server" Text="确定" />
<input id="Button6" type="button" class="C_inputs" onclick="ShowDiv('DayChang_<%#Eval("ID") %>" value="取消" />
</li>
</ItemTemplate>
</asp:Repeater>
</ul></div></td></tr>
...
<ItemTemplate> 
<li><span><%#Eval("CreateTime","{0:MM-dd hh:mm}") %>  </span><a href="/UploadFiles/<%#Eval("Content") %>"><%#Eval("Title") %><%#Eval("Content") %></a> <div>  <asp:LinkButton ID="lbtDel" CommandArgument='<%#Eval("ID") %>' CommandName="SetDel" Text="删除" runat="server" ></asp:LinkButton> 
<%--<asp:LinkButton ID="lbtDate" CommandArgument='<%#Eval("ID") %>' CommandName="SetDate" Text="延期到" runat="server" ></asp:LinkButton> --%>
</div>    </li>
